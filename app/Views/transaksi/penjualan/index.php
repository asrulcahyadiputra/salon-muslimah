                        ...
                        <tbody>
                            ...
                            foreach ($pnj as $key => $value) { ?>
                                <tr>
                                    ...
                                    <td><?= mediumdate_indo($value->tgl_transaksi) ?></td>
                                    ...
                                    <td class="text-center action"><?= ($value->status == 'selesai') ? '<span class="badge badge-success">Sudah Bayar</span>' : '<span class="badge badge-warning">' . ucwords($value->status) . '</span>' ?></td>
                                    <!-- <td>
...
<?= $this->include('transaksi/penjualan/detail'); ?>
...
<script>
    ...
    $('#tablemenu').on('click', 'tr', function(e) {
        if (!$(e.target).closest('.action').length) {
            ...
            $.ajax({
                type: 'get',
                url: '<?= base_url("Transaksi/detailPenjualan") ?>',
                data: {
                    id_transaksi: id
                },
                dataType: 'json',
                success: (res) => {
                    ...
                    html += `</tbody></table>`;
                    let url = "<?= base_url('Transaksi/cetakPenjualan') ?>/" + data_m.id_transaksi;
